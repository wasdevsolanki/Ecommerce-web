...
class ProductController extends Controller
{
    ...
    public function productStore(ProductRequest $request){
        ...
        if($request->child_category != null){
            $request['en_category_name'] = $request['child_category'];
        }

        $request['price'] = $request['commission_price'];
        ...
        $total = 0;
        foreach ($request['qtybysize'] as $key => $value) {
            $total += $value;
        }

        $request['qty'] = $total;
        $request['size'] = $allsize;
        ...
        if (!empty($request->design_image)) {
            $data['design_image'] = fileUpload($request['design_image'], ProductImage());
        }else {
            return redirect()->back()->with('toast_error', __('Image is  required'));
        }

        $data['qtybysize'] = $request->qtybysize;
        $data['clothing_type'] = $request->clothing_type;
    ...
    public function physicalProductAdd($data)
    {
        ...
        $product = Product::create([
            ...
            'fr_Product_Name' => $data['fr_product_name'] ?? 'Not Found',
            'fr_Product_Slug' => $data['fr_product_slug'] ?? 'Not Found',
            'fr_About' => $data['fr_about'] ?? 'Not Found',
            'fr_Description' => $data['fr_description'] ?? 'Not Found',
            'fr_ShippingReturn' => $data['fr_shippingreturn'] ?? 'Not Found',
            'fr_AdditionalInformation' => $data['fr_additionalinformation'] ?? 'Not Found',
            'Quantity' => $data['qty'],
        ...
        if(!empty($product)){
            ...
            // store getbysize start
            foreach($data['qtybysize'] as $key=>$value) {
                Stock::create([
                    'product_id' => $product->id,
                    'size_id' => $key,
                    'quantity'=>$value,
            ...
            Activity::create([
                'log_name' => 'Product Create',
                'description' => "Product was {$Model->en_Product_Name} Created",
                'subject_id' => $Model->id,
    ...
    public function productDelete($id){
        ...
        if ($delete) {
            ...
            Activity::create([
                'log_name' => 'Product Delete',
                'description' => "Product was {$Model->en_Product_Name} Deleted",
                'subject_id' => $Model->id,
                'subject_type' => get_class($Model),
                'causer_id' => auth()->id(), // ID of the user who caused the activity
                'causer_type' => get_class(auth()->user()), // Class name of the user model
    ...
    public function physicalProductUpdate($data, $product)
    {
        ...
        if(!empty($update)){
            ...
            $Model = Product::find($product->id);
            Activity::create([
                'log_name' => 'Product Update',
                'description' => "Product was {$Model->en_Product_Name} Updated",
                'subject_id' => $Model->id,
                'subject_type' => get_class($Model),
                'causer_id' => auth()->id(), // ID of the user who caused the activity
                'causer_type' => get_class(auth()->user()), // Class name of the user model
    ...
    public function getProductlist(Request $request){
        ...
        $validSecretKey = 'nopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ';
        ...
        foreach ($products as $product) {
            $item = [
                ...
                'price' => $product->Price,
                'brand' => $product->brand->en_BrandName, // Assuming the Brand model has a 'name' attribute
                'category' => $product->category->en_Category_Name, // Assuming the Category model has a 'name' attribute
                'colors' => $product->colors->pluck('Name'), // Assuming the Color model has a 'name' attribute
                'sizes' => $product->sizes->pluck('Size'), // Assuming the Size model has a 'name' attribute
            ];
