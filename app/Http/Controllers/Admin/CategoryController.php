...
//                })
                ...
                ->rawColumns(['action', 'Category_Name', 'Category_Slug','Parent','Status','Description'])
                ->make(true);
        }
    ...
    public function categoryStore(CategoryRequest $request){
        ...
        if( is_null($request->en_parent_category) ){
            $category=Category::create([
                'en_Category_Name'=>$request->en_category_name,
                'en_Description'=>$request->en_description,
                'en_Category_Slug'=>$this->slugify($request->en_category_name),
                'fr_Category_Name'=>$request->fr_category_name ?? 'Not Found',
                'fr_Description'=>$request->fr_description ?? 'Not Found',
                'fr_Category_Slug'=>$this->slugify($request->fr_category_name) ?? 'Not Found',
                'Category_Icon'=>$request->icon_class,
            ]);
        }else{
            $category=Category::create([
                'en_Category_Name'=>$request->en_category_name,
                'parent_id'=> $request->en_parent_category,
                'en_Description'=>$request->en_description,
                'en_Category_Slug'=>$this->slugify($request->en_category_name),
                'fr_Category_Name'=>$request->fr_category_name ?? 'Not Found',
                'fr_Description'=>$request->fr_description ?? 'Not Found',
    ...
    {
        // Query the database to get child categories based on the selected category
        $childCategories = Category::where('parent_id', $category)->get();
