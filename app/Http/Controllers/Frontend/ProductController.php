...
class ProductController extends Controller
{
    ...
    public function allProduct(){
        ...
        $products =Product::with('brand','category','colors','sizes','product_tags')->latest()->paginate(15);
        $data['products'] = $products;
    ...
    public function CategoryWiseProduct($id){
        ...
        $data['category']=Category::with('products')->where('en_Description',null)->orWhere('Category_Icon',null)->get();
        $data['brands']=Brand::with('products')->get();
        $products=Product::with('brand','category','colors','sizes','product_tags')->where('Category_Id',$id)->latest()->paginate(15);
        $data['products'] = $products;
        $seo = SeoSetting::where('slug', 'all-products')->first();
        $data['title'] = $seo->title;
        $data['description'] = $seo->description;
    ...
    public function CategoryWiseProductFind($category){
        $category = Category::where('en_Category_Name', $category)->first();
        $id = $category->id;
        $data['category_m'] = Category::whereId($id)->first();
        $data['tags']=ProductTag::with('product')->latest()->get();
        $data['colors']=Color::with('products')->latest()->get();
        ...
        $data['brands'] = Brand::whereHas('products', function ($query) use ($id) {
        ...
        if($products->isEmpty()){
            $child_category_list = Category::select('id')->where('parent_id', $id)->get()->toArray();
            $products=Product::with('brand','category','colors','sizes','product_tags')->whereIn('Category_Id',$child_category_list)->latest()->paginate(15);
        }
    ...
    public function BrandWiseProductFind($brand){
        $item = Brand::where('en_BrandName',$brand)->first();
        $id = $item->id;
        ...
        $data['category']=Category::with('products')->where('en_Description',null)->orWhere('Category_Icon',null)->get();
        $data['brands']=Brand::with('products')->get();
        $products=Product::with('brand','category','colors','sizes','product_tags')->where('Brand_Id',$id)->latest()->paginate(15);
        $data['products'] = $products;
        $seo = SeoSetting::where('slug', 'all-products')->first();
        $data['title'] = $seo->title;
        $data['description'] = $seo->description;
    ...
    public function CategoryWiseProductLeftFind($category){
        $category = Category::where('en_Category_Name', $category)->first();
        $id = $category->id;
        $data['category_m'] = Category::whereId($id)->first();
        $data['tags']=ProductTag::with('product')->latest()->get();
    ...
    public function BrandWiseProductLeftFind($brand){
        $item = Brand::where('en_BrandName',$brand)->first();
        $id = $item->id;
        $data['category_m'] = Brand::whereId($id)->first();
        $data['tags']=ProductTag::with('product')->latest()->get();
    ...
    public function CategorySearchProduct(Request $request){
        ...
        if(!is_null($request->category)) {
            $products = $search_query->where('Category_Id',$cat)->latest()->paginate(15);
        }else {
            $products = $search_query->latest()->paginate(15);
        }
        if($products->count() == 0) {
