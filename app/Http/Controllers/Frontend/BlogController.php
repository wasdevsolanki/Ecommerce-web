...
class BlogController extends Controller
{
    ...
    public function blogDetailBySlug($slug){

        $blog_id=Blog::where('slug',$slug)->first();
        $blog=Blog::where('slug',$slug)->orWhere('id', $blog_id->id)->with('comments', 'author')->first();
        $data['blog'] = $blog;
        $data['comment'] = BlogComment::where('Blog_Id',$blog_id->id)->count('id');
        $data['title'] = $blog->en_Title;
        $data['description'] = $blog->en_Title;
        $data['keywords'] = $blog->en_Title;
