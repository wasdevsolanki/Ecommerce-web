...
class CheckoutController extends Controller
{
    ...
    public function orderCreateCall($order_number, $shipping_charge, $tax, $subtotal, $discount, $grand_total, $payment_method, $txn = null)
    {
        ...
        if($order['success'] == true) {
            ...
            $getOrderDetails = OrderDetails::where('Order_Id', $getOrder->id)->get();

            if ($getOrderDetails->isNotEmpty()) {
                $dataset = ['Order' => $getOrderDetails->toArray()];
                $response = Http::get('http://inventory.malakofftraders.com/api/order/notify', $dataset);
            }
        ...
        if ($order){
            ...
            foreach ($content as $item){
                $this->subQtyProduct($item->id, $item->qty);
                    ...
                    Design::where('id',$item->options->design_image)->update(['order_id' => $order->id]);
                }
                ...
                OrderDetails::create([
                    ...
                    'Sizes'=> json_encode($item->options->sizebyqty) ?? '',
                    'Quantity'=>$item->qty,
