                        ...
                        <div class="modal-body">
                            ...
                            <div class="input__group">
                                ...
                                <select name="Order_Status" id="order-status" class="form-select">
                                    ...
                                    <option  data-toggle="collapse" data-target="#additionalContent" value="{{ORDER_SHIPPED}}" {{$order->Order_Status == ORDER_SHIPPED ? 'selected disabled' : ''}}>{{__('Shipped')}}</option>
                            ...
                            <hr>
                            <p>
                                <a class="btn p-2 btn-primary mt-2" data-bs-toggle="collapse" href="#collapseExample" role="button" aria-expanded="false" aria-controls="collapseExample">
                                    Shipping Details
                                </a>
                            </p>
                            <div class="collapse" id="collapseExample">
                                <div class="input__group mt-2" id="exampleModalToggle2" aria-hidden="true" aria-labelledby="exampleModalToggleLabel2" tabindex="-1">
                                    <div class="input__group" id="dynamicFieldsContainer">
...
{{--                            <div class="input__group mt-2" id="exampleModalToggle2" aria-hidden="true" aria-labelledby="exampleModalToggleLabel2" tabindex="-1">--}}
{{--                                <div class="input__group" id="dynamicFieldsContainer"></div>--}}
{{--                            </div>--}}
                        </div>
    ...
    @push('post_scripts')
        ...
        <script>
            var shippingMethods = <?php  echo json_encode(ShippingMethod()); ?>;
            document.addEventListener('DOMContentLoaded', function() {
                const orderStatusSelect = document.getElementById('order-status');
                const dynamicFieldsContainer = document.getElementById('dynamicFieldsContainer');

                orderStatusSelect.addEventListener('change', function() {
                    console.log("dynamicFieldsContainer");
                    const selectedStatus = orderStatusSelect.value;
                    dynamicFieldsContainer.innerHTML = ''; // Clear previous dynamically generated elements

                    if (selectedStatus === "{{ ORDER_SHIPPED }}") {
                        // If the "ORDER_SHIPPED" status is selected, create the new select box
                        const newSelectBox = document.createElement('select');
                        newSelectBox.name = "shipping_method"; // Replace with the actual name for the new select box
                        newSelectBox.id = "shippingMethodSelect";
                        newSelectBox.className = "form-select mb-2";
                        // Loop through the shipping methods and create options
                        shippingMethods.forEach((method) => {
                            const option = document.createElement('option');
                            option.value = method.id; // Set the value to the tracking URL
                            option.textContent = method.name; // Set the display text to the method name
                            newSelectBox.appendChild(option);
                        });

                        // Create the input field for tracking ID
                        const trackingInput = document.createElement('input');
                        trackingInput.type = 'text';
                        trackingInput.name = 'tracking_id'; // Replace with the actual name for the input field
                        trackingInput.placeholder = 'Enter Tracking ID';
                        trackingInput.className = 'form-control mb-2';

                        // Append both the new select box and input field to the container
                        dynamicFieldsContainer.appendChild(newSelectBox);
                        dynamicFieldsContainer.appendChild(trackingInput);
                    }
                });
            });
